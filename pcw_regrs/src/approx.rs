...
use crate::{euclid_sq_metric, DegreeOfFreedom, OFloat, SegmentModelSpec, ValidTimeSeriesSample};

/// Models a timeseries via a polynomial function.
#[derive(new, Debug, Eq, PartialEq, Clone)]
pub struct PolynomialApproximator {
    dof: DegreeOfFreedom,
    poly: OwnedNewtonPolynomial<OFloat, OFloat>,
    training_error: OFloat,
}
...
impl PolynomialApproximator {
    ...
    pub fn fit_to_data(timeseries: &ValidTimeSeriesSample, dof: DegreeOfFreedom) -> Self {
        let PolyFit {
            polynomial,
            residual,
    ...
    pub fn prediction(&self, prediction_time: &OFloat) -> OFloat {
...
fn approximation_on_segment(
    timeseries: &ValidTimeSeriesSample,
    segment_start_idx: usize,
    segment_stop_idx: usize,
    dof: DegreeOfFreedom,
...
/// Compute the full piecewise function mapping time indices to the corresponding models.
pub fn full_approximation_on_seg<'a>(
    timeseries: &'a ValidTimeSeriesSample<'a>,
    segment_start_idx: usize,
    segment_stop_idx: usize,
    segment_models: impl IntoIterator<Item = DegreeOfFreedom>,
    cuts: impl IntoIterator<Item = usize>,
) -> VecPcwFn<usize, PolynomialApproximator> {
    let mut models = segment_models.into_iter();
    let (mut cuts, approximations): (Vec<_>, Vec<_>) = cuts
        .into_iter()
        .chain(std::iter::once(segment_stop_idx))
